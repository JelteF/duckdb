...
from transformer_trampoline_config import (
    TrampolineRuleMode,
    load_transformer_trampoline_config,
)
...
matcher_cpp_path = src_dir / "parser" / "peg" / "matcher_factory.cpp"
transformer_factory_cpp_path = transformer_dir / "peg_transformer_factory.cpp"
matcher_override_cpp_path = src_dir / "parser" / "peg" / "compiled_grammar.cpp"
...
TRANSFORM_FLAGS_START_BLOCK = SEPARATOR + "\t// START GENERATED CHILDLESS TRANSFORM RULES\n" + SEPARATOR
TRANSFORM_FLAGS_END_BLOCK = SEPARATOR + "\t// END GENERATED CHILDLESS TRANSFORM RULES\n" + SEPARATOR
...
#: An initializer that reserves no child slots. Since no finalizer pushes children, such a rule runs to completion in
#: one step and TransformStack can run it without a frame of its own.
CHILDLESS_INITIALIZE_BODY = re.compile(r"\{\n\tprocess\.ReserveChildSlots\(0\);\n\}")


def derive_childless_rules(sources, rule_names):
    """Rule names whose transformer never asks for a child."""
    derived = []
    for rule_name in rule_names:
        marker = f"PEGTransformerFactory::{init_name(rule_name)}(PEGTransformer &"
        for source in sources:
            start = source.find(marker)
            if start == -1:
                continue
            body_start = source.find("{", start + len(marker))
            if body_start != -1 and CHILDLESS_INITIALIZE_BODY.match(source, body_start):
...
class ManualTransformRegistry:
    def __init__(self, paths):
        self.signatures = {}
        self.declarations = {}
        self._load(sorted(paths))

    @staticmethod
    def _signature_start(text, match_start):
        line_start = text.rfind("\n", 0, match_start) + 1
        if text[line_start:match_start].strip():
            return line_start
        previous_line_end = line_start - 1
        while previous_line_end > 0:
        ...
        for path in paths:
            ...
            for match in pattern.finditer(text):
                ...
                if not next_token.startswith("{"):
                    continue
                signature_start = self._signature_start(text, match.start())
                signature = text[signature_start : paren_end + 1].strip()
                rule_name = match.group(1)
                self.signatures.setdefault(rule_name, set()).add(self._signature_key(signature))
                declaration = signature.replace("PEGTransformerFactory::", "", 1)
                declaration = "\tstatic " + declaration.replace("\n", "\n\t") + ";\n"
                self.declarations.setdefault(rule_name, []).append(declaration)

    @staticmethod
    def _signature_key(signature):
        prefix = "PEGTransformerFactory::"
        signature = norm_ws(signature)
    ...
    def get_declarations(self, rule_name):
        return self.declarations.get(rule_name, ())
...
class UseGramPreviewEmitter:
    ...
    def emit_header_declarations(self):
        ...
        for rule_name in self.emitted_rules():
            lines.extend(manual_transform_registry().get_declarations(rule_name))
        return "".join(lines)
    ...
    def classify_rule(self, rule_name, rule):
        ...
        if rule_name in self.matcher_overrides and rule_name in self.rule_types:
            ...
            return RuleCapability(
                rule_name,
                RuleCapabilityStatus.GENERATED,
                "provided by matcher_rule_overrides",
            )
        if rule_name in self.matcher_overrides:
            return RuleCapability(
                rule_name,
                RuleCapabilityStatus.PROVIDED,
                "provided by matcher_rule_overrides",
            )
        if rule_name in self.excluded_rules:
            return RuleCapability(
                rule_name,
                RuleCapabilityStatus.EXCLUDED,
                "excluded by grammar_types.yml",
            )
        try:
        ...
        if literal_string_values(ast) is None and rule_name not in self.rule_types:
            return RuleCapability(
                ...
                "no return type in grammar_types.yml",
                ast,
            )
    ...
    def validate_grammar_types(self):
        ...
        if not missing:
            return
        print(
            "Error: missing trampoline transformer metadata in grammar_types.yml:",
            file=sys.stderr,
        )
        for capability in missing:
            print(f"  {capability.rule_name}: {capability.reason}", file=sys.stderr)
        print(
            "Add type metadata for these rules or explicitly exclude syntax-only rules.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    def emit_special_dispatch_rules(self):
        ...
        for rule_name, dispatch in SPECIAL_DISPATCH_RULES.items():
            ...
            lines.append("\t\tresult->named_param_map = transformer.named_parameter_map;")
            lines.append("\t}")
            lines.append("\tresult->has_anonymous_parameters = transformer.has_anonymous_parameters;")
    ...
    def emit_matcher_override_rule(self, rule_name):
        result_expr = self.matcher_transform_expr(rule_name, "process.parse_result")
        lines = []
        lines.append(
            f"void PEGTransformerFactory::{init_name(rule_name)}(PEGTransformer &transformer, "
            f"GeneratedTransformProcess &process) {{"
    ...
    def is_identifier_matcher(self, rule_name):
        return self.matcher_overrides[rule_name].matcher in (
            "identifier",
            "reserved_identifier",
        )
    ...
    def emit_syntax_only_rule(self, rule_name, ast):
        ...
        if self.is_parse_result_syntax_only_rule(rule_name, ast):
            if isinstance(ast, ChoiceNode):
                ...
                lines.append(f"\tauto result = Transform{rule_name}(transformer, process.parse_result);")
    ...
    def emit_choice_rule(self, rule_name, ast):
        ...
        if self.is_parse_result_manual_choice_rule(rule_name, ast):
            ...
            lines.append("\tif (process.child_results[0]) {")
            ...
            if child_cpp_type == "string":
                ...
                lines.append("\tif (process.child_results[0]) {")
                lines.append("\t\tchild = process.TakeResult<string>(0);")
                lines.append("\t} else {")
                lines.append("\t\tauto &list_pr = process.parse_result.Cast<ListParseResult>();")
                lines.append("\t\tauto &choice_result = list_pr.Child<ChoiceParseResult>(0).GetResult();")
                ...
                lines.append(f"\tauto child = process.TakeResult<{child_cpp_type}>(0);")
            ...
            lines.append("\tif (process.child_results[0]) {")
            lines.append("\t\tresult = process.TakeResult<string>(0);")
            lines.append("\t} else {")
            lines.append("\t\tauto &list_pr = process.parse_result.Cast<ListParseResult>();")
            lines.append("\t\tauto &choice_result = list_pr.Child<ChoiceParseResult>(0).GetResult();")
            lines.append("\t\tif (choice_result.type == ParseResultType::IDENTIFIER) {")
            ...
            lines.append("\tif (process.child_results[0]) {")
            lines.append("\t\tresult = process.TakeResult<Identifier>(0);")
            lines.append("\t} else {")
            lines.append("\t\tauto &list_pr = process.parse_result.Cast<ListParseResult>();")
            lines.append("\t\tauto &choice_result = list_pr.Child<ChoiceParseResult>(0).GetResult();")
            lines.append("\t\tif (choice_result.type == ParseResultType::IDENTIFIER) {")
    ...
    def emit_choice_initialize(self, rule_name, ast):
        ...
        lines.append("\tprocess.ReserveChildSlots(1);")
        lines.append("\tauto child_rule = choice_result.GetRule();")
        lines.append("\tauto has_transform_process = child_rule && child_rule->HasTransform();")
        syntax_only_alternatives = self.choice_syntax_only_alternative_names(ast)
        child_cpp_types = set()
        for alternative in ast.alternatives:
            if isinstance(alternative, ReferenceNode) and alternative.name in self.rule_types:
    ...
    def emit_sequence_rule(self, rule_name, ast):
        ...
        for arg in plan.finalize_args:
            if isinstance(arg, DirectParseArg):
                ...
                lines.append(f"\tauto {var_name} = process.TakeResult<{self.cpp_type(arg.rule_name)}>({slot_expr});")
                ...
                lines.append(f"\tif (process.child_results[{slot_expr}]) {{")
                value_expr = f"process.TakeResult<{self.cpp_type(arg.rule_name)}>({slot_expr})"
                ...
                lines.append(f"\t\tauto slot_idx = process.child_results.size() - 1;")
                value_expr = f"process.TakeResult<{self.cpp_type(arg.rule_name)}>(slot_idx)"
                lines.append(f"\t\t{var_name} = {value_expr};")
                lines.append("\t}")
                arg_names.append(self.transform_arg_expr(arg.rule_name, var_name))
                ...
                lines.append(
                    f"\t\t\t{var_name}_value.push_back(process.TakeResult<{self.cpp_type(arg.rule_name)}>(i));"
                )
                lines.append("\t\t}")
                ...
                loop_end = f"{arg.slot_start} + dynamic_child_count"
                lines.append(f"\tfor (idx_t i = {arg.slot_start}; i < {loop_end}; i++) {{")
                lines.append(f"\t\t{var_name}.push_back(process.TakeResult<{self.cpp_type(arg.rule_name)}>(i));")
                lines.append("\t}")
                arg_names.append(self.transform_arg_expr(arg.rule_name, var_name))
        ...
        if dynamic_child:
            if plan.list_child:
                ...
                if trailing_optional:
                    ...
                    lines.append(
                        f"\tprocess.ReserveChildSlots({logical_child_slots} + dynamic_child_count - 1 + ({trailing_optional.var_name}_opt.HasResult() ? 1 : 0));"
                    )
                    ...
                    lines.append(f"\tprocess.ReserveChildSlots({logical_child_slots} + dynamic_child_count - 1);")
                    for stack_child in reversed(frame_children_after_dynamic):
                        self.emit_stack_child_push(lines, plan, stack_child)
                ...
                lines.append("\t\t}")
                lines.append("\t} else {")
                lines.append(f"\t\tprocess.ReserveChildSlots({logical_child_slots} - 1);")
                for stack_child in reversed(frame_children_after_dynamic):
                    self.emit_stack_child_push(lines, plan, stack_child, "\t\t")
                lines.append("\t}")
            elif plan.required_repeat_child:
            ...
            lines.append(f"\tprocess.ReserveChildSlots({fixed_child_slots});")
...
def matcher_override_expr(rule_name, override):
    matcher = override.get("matcher")
    suggestion = override.get("suggestion")
    if matcher == "identifier" and suggestion:
        return f"make_uniq<IdentifierMatcher>(SuggestionState::{suggestion}, keyword_helper)"
    if matcher == "reserved_identifier" and suggestion:
...
def replace_generated_block(content, start_block, end_block, generated_content, path):
    start_idx = content.find(start_block)
    if start_idx == -1:
        raise RuntimeError(f"Could not find generated block start marker in {path}")
    end_idx = content.find(end_block, start_idx + len(start_block))
    if end_idx == -1:
        raise RuntimeError(f"Could not find generated block end marker in {path}")
    block_end = end_idx + len(end_block)
    return content[:start_idx] + start_block + generated_content + end_block + content[block_end:]
...
def write_matcher_rule_overrides(matcher_overrides):
    ...
    for rule_name, override in matcher_overrides.items():
        ...
        expression = matcher_override_expr(rule_name, override)
        line = f'\tAddTerminalRuleOverride(overrides, "{rule_name}", {expression});\n'
    ...
    content = replace_generated_block(
        ...
        MATCHER_END_BLOCK,
        "".join(lines),
        matcher_override_cpp_path,
    )
    matcher_override_cpp_path.write_text(content)
    print(f"Updated {matcher_override_cpp_path}")


def write_transform_frame_flags(childless_rules):
    content = transformer_factory_cpp_path.read_text()
    lines = [f'\tgrammar.SetChildlessTransform("{rule_name}");\n' for rule_name in childless_rules]
    content = replace_generated_block(
        content,
        TRANSFORM_FLAGS_START_BLOCK,
        TRANSFORM_FLAGS_END_BLOCK,
        "".join(lines),
...
def main():
    ...
    arg_parser.add_argument(
        "--report",
        action="store_true",
        help="Print per-rule trampoline generation status.",
    )
    arg_parser.add_argument(
        "--grammar",
        action="append",
    ...
    matcher_override_config = load_matcher_rule_overrides(grammar_types_file)
    ...
    if args.write:
        ...
        manual_sources = [path.read_text() for path in sorted(transformer_dir.glob("transform_*.cpp"))]
        childless_rules = derive_childless_rules([source] + manual_sources, all_rules.keys())
        write_transform_frame_flags(childless_rules)
    elif args.report:
...
def write_hpp(declarations, result_types):
    hpp_path = include_peg_dir / "peg_transformer.hpp"
    content = hpp_path.read_text()
    content = replace_generated_block(
        content,
        RESULT_TYPE_START_BLOCK,
        RESULT_TYPE_END_BLOCK,
        result_types,
        hpp_path,
    )
    content = replace_generated_block(
