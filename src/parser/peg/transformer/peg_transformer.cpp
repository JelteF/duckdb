...
TransformStep TransformStep::Complete(transform_result_ptr result) {
	D_ASSERT(result);
...
transform_result_ptr TransformStep::TakeResult() {
	D_ASSERT(!child);
	D_ASSERT(result);
...
GeneratedTransformProcess::GeneratedTransformProcess(PEGTransformer &transformer_p, TransformInput input,
    ...
    : parse_result(input.parse_result), child_results(transformer_p.GetAllocator()), info(info_p),
...
TransformStep FinalizeTransformProcess::Resume(transform_result_ptr child_result) {
	D_ASSERT(!completed);
...
TransformStep GeneratedTransformProcess::Resume(transform_result_ptr child_result) {
	D_ASSERT(!completed);
...
transform_result_ptr TransformStack::ExecuteFrame(TransformStackFrame &frame) {
	if (!frame.process) {
...
transform_result_ptr PEGTransformer::TransformInternal(ParseResult &parse_result) {
