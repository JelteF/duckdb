...
//                         DuckDB
//
// duckdb/parser/peg/matcher/precedence_ladder.hpp
//
//
//===----------------------------------------------------------------------===//
...
//! The form a precedence ladder level is written in. Not serialized, so it is on the blacklist in
...
//! One level of the operator precedence ladder: a collapsible rule of the form `X <- Y`, `X <- Y Tail*`,
...
//! frames then finds that no tail follows. Matching the chain as a ladder instead parses the operand once and walks
